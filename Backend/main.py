...
from handlers.loan_handler import fast_track_approve, approve_loan, reject_loan
from handlers.recovery_handler import move_stage
from handlers.auction import handle_auction_action
...
# ─────────────────────────────────────────────────────────────────
# Group membership registry  uid → groupId
# ─────────────────────────────────────────────────────────────────
_uid_groups: dict[str, str] = {}
_uid_groups_lock = threading.Lock()

def _register_group(uid: str, group_id: str) -> None:
    with _uid_groups_lock:
        _uid_groups[uid] = group_id

def _unregister_group(uid: str) -> None:
    with _uid_groups_lock:
        _uid_groups.pop(uid, None)
...
async def _broadcast_to_group(group_id: str, channel: str, event: str, payload) -> None:
    """Broadcast a message only to connections whose uid is in group_id."""
    with _uid_groups_lock:
        group_uids = {uid for uid, gid in _uid_groups.items() if gid == group_id}
    with _connections_lock:
        sockets = [
            ws
            for uid, ws_set in _connections.items()
...
async def send_snapshots(ws) -> None:
    ...
    # Send current active / scheduled auction (if any)
    try:
        active_auctions = await loop.run_in_executor(
            None,
            lambda: list(
        ...
        if active_auctions:
            ...
            for k, v in data.items():
                if v is None:
                    ...
                    safe[k] = v
            await _send(ws, "auction_room", "auction_snapshot", safe)
    except Exception as exc:
        log.warning(f"  ⚠ Could not send active auction snapshot: {exc}")
...
def setup_listeners(loop: asyncio.AbstractEventLoop) -> list:
    ...
    # users collection — broadcast kycStatus / userType changes to all clients
    # so members receive a secondary WS push on top of Firestore onSnapshot
    def _on_users_snapshot(col_snapshot, changes, read_time):
        for change in changes:
            if change.type.name in ("MODIFIED", "ADDED"):
                doc  = change.document
                data = doc.to_dict() or {}
                _throttle_broadcast(loop, "users_update", "status_change", {
                    "uid":        doc.id,
                    "kycStatus":  data.get("kycStatus"),
                    "userType":   data.get("userType"),
                    "kycRejectionReason": data.get("kycRejectionReason", ""),
                })
    ...
    try:
        if channel == "kyc_queue":
            ...
            if action == "fast_track":
                ...
                updated = await approve_loan(doc_id, uid, display_name)
                await _broadcast("loan_inbox", "action_ack", {
                    "id": doc_id, "new_status": "Approved", "doc": updated
                })
            elif action in ("reject", "rejected"):
                reason  = payload.get("reason", "No reason provided")
                updated = await reject_loan(doc_id, reason, uid, display_name)
                await _broadcast("loan_inbox", "action_ack", {
                    "id": doc_id, "new_status": "Rejected",
                    "rejection_reason": reason, "doc": updated
                })
        ...
        elif channel == "auction_room":
            await handle_auction_action(
                ws, action, payload, uid, display_name,
                _send, _broadcast_to_group, _connections
            )
...
async def handler(ws) -> None:
    ...
    # 2b. Resolve and cache this user's groupId for group broadcasts
    try:
        loop = asyncio.get_event_loop()
        ...
        if u_doc.exists:
            grp = u_doc.to_dict().get("groupId", "")
            if grp:
                _register_group(uid, grp)
                log.info(f"  👥 uid={uid} → group={grp}")
    except Exception as exc:
        log.warning(f"  ⚠ Could not resolve groupId for {uid}: {exc}")
