...
export function useWebSocket() {
  ...
  const routeMessage = useCallback((msg) => {
    ...
    if (channel === 'auction_room') {
      // All auction_room events go into wsStore so useAuction can react
      setAuctionEvent(event, payload ?? {});
      return;
    }

    if (channel === 'users_update') {
      // KYC status updates — no store action needed, Firestore onSnapshot handles UI
      return;
    }
